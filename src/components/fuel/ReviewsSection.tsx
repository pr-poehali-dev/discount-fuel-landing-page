...
const E1 = "hsl(158,70%,46%)";
const E2 = "hsl(170,65%,44%)";

const REVIEWS = [
  {
    name: "Виктор С.", city: "Москва",      badge: "Газпромнефть", color: E1,
    ...
    rating: 5, date: "5 апреля 2026",
  },
  {
    name: "Андрей М.", city: "Красногорск", badge: "Роснефть",     color: E2,
    ...
    rating: 5, date: "28 марта 2026",
  },
  {
    name: "Игорь Т.",  city: "Балашиха",    badge: "Газпромнефть", color: E1,
    ...
    rating: 5, date: "20 марта 2026",
  },
  {
    name: "Наталья П.", city: "Подольск",   badge: "Обе сети",     color: E1,
    ...
    rating: 5, date: "15 марта 2026",
  },
  {
    name: "Светлана В.", city: "Химки",     badge: "Роснефть",     color: E2,
    ...
    rating: 5, date: "10 марта 2026",
  },
  {
    name: "Роман К.", city: "Одинцово",     badge: "Газпромнефть", color: E1,
    ...
    <section id="reviews" ref={ref} className="py-24 px-6 fuel-grid">
      <div className="max-w-6xl mx-auto">
        <div className={`text-center mb-14 opacity-0 ${inView ? "animate-fade-up" : ""}`}>
          ...
          <p className="text-[hsl(155,10%,52%)] mt-3 text-sm">Более 5 000 клиентов уже экономят на дизеле</p>
        </div>
          ...
          {REVIEWS.map((r, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full font-semibold text-[hsl(160,12%,5%)]" style={{ background: r.color }}>
                  {r.badge}
                </span>
              </div>

              <p className="text-sm text-[hsl(150,12%,78%)] leading-relaxed flex-1">"{r.text}"</p>

              <div className="flex items-center justify-between mt-5 pt-4 border-t border-[hsl(160,8%,14%)]">
                <div>
                  <div className="font-semibold text-sm text-white">{r.name}</div>
                  <div className="text-xs text-[hsl(155,10%,48%)]">{r.city}</div>
                </div>
                <div className="text-xs text-[hsl(155,10%,42%)]">{r.date}</div>
              </div>
            </div>
          ))}
        </div>
